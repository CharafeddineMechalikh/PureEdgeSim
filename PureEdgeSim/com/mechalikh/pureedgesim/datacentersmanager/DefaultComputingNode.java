...
public class DefaultComputingNode extends LocationAwareNode {
	...
	public DefaultComputingNode(SimulationManager simulationManager, double mipsPerCore, long numberOfCPUCores,
		...
		this.setAvailableStorage(storage);
		this.setTotalMipsCapacity(mipsPerCore * numberOfCPUCores);
		this.mipsPerCore = mipsPerCore;
		this.setNumberOfCPUCores(numberOfCPUCores);
	...
	public double getCurrentCpuUtilization() {
		if (this.getTotalMipsCapacity() == 0)
			return 0;
		return tasks / getTotalMipsCapacity() > 1 ? 100 : tasks * 100 / getTotalMipsCapacity();
	}
	...
	public void submitTask(Task task) {
		...
		task.setArrivalTime(getSimulator().clock());
	...
	private void startExecution(Task task) {
		...
		availableCores--;
		task.setExecutionStartTime(getSimulator().clock());
		 ...
		 * 
		 * I mean, this makes sense right?. The problem with this is that it will
		 * depend on the update interval. To get more accurate results, you need to
		 * set the update interval as low as possible, this will in turn  increase the
		 * simulation duration, which is clearly not convenient. One way around it, is
		 * to make the measurement here, when the task is being executed. 
		 * The problem with this is that if we don't receive a task, the static energy consumption will not be measured.
		 * So the best approach is to measure the dynamic one here, and add the static one there.
		 */

		getEnergyModel().updateDynamicEnergyConsumption(task.getLength(),this.getMipsCapacity());
	...
	private void executionFinished(Event e) {
		...
		((Task) e.getData()).setExecutionEndTime(this.getSimulator().clock());
		...
		if (!getTasksQueue().isEmpty()) {
			...
			task.setExecutionStartTime(this.getSimulator().clock());

			// Reserve a CPU core for this task.
			availableCores--;
