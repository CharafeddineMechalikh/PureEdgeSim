...
public class EdgeDevicesParser extends ComputingNodesParser {
	...
	protected boolean typeSpecificChecking(Document xmlDoc) {
		...
		for (int i = 0; i < devicesList.getLength(); i++) {
			...
			if (!checkEdgeDevice(deviceElement))
				return false;
		...
		if (percentage != 100 && type == TYPES.EDGE_DEVICE) {
			throw new IllegalArgumentException(getClass().getSimpleName()
					+ " - Check the edge_devices.xml file!, the sum of percentages must be equal to 100%.");
		}
		return true;
	}

	private boolean checkEdgeDevice(Element deviceElement) {
		...
		assertDouble(deviceElement, "percentage", value -> (value > 0), "> 0. Check the " + file + " file!");
